...
export const generateMetadata = async ({ params }: Props): Promise<Metadata> => {
	...
	return {
		...buildContentMetadata({
			title,
			description,
			ogImageParams: { type: 'componentsArticle', title, description },
		}),
		alternates: {
...
const Page = async ({ params }: Props) => {
	...
	const components = metadata.category ? getContentByCategory(metadata.category) : [];

	return (
		<>
			<script
				dangerouslySetInnerHTML={{
					__html: JSON.stringify(getPageJsonLd(component)).replace(/</g, '\\u003c'),
				}}
				type="application/ld+json"
			/>
			<ArticleNavBar description="tous les composants" item={component} items={components} slug={slug} useLlm={false} />
