...
export const InlineToc = ({
	...
	return (
		<CollapsibleWithContext
			className={cn('not-prose rounded-lg bg-code', className)}
			{...props}
		>
			<CollapsibleTrigger className="group/toc inline-flex w-full cursor-pointer items-center gap-2 p-3 pl-4 font-medium text-sm [&_svg]:size-4">
				...
				<div aria-hidden className="ml-auto shrink-0 text-muted-foreground">
					<CollapsibleChevronsIcon />
				</div>
			</CollapsibleTrigger>

			<CollapsibleContent className="overflow-hidden duration-300 data-[state=closed]:animate-collapsible-fade-up data-[state=open]:animate-collapsible-fade-down">
				<ul className="flex flex-col px-4 pb-3 text-foreground">
					{items.map((item) => (
						<li
							className="flex py-1"
							key={item.url}
