...
export const ToolItem = memo(({ tool, isLast = false, isDescription = false }: ToolItemProps) => {
		...
		<Link aria-label={metadata.title} href={`/utils/${slug}`} prefetch={false}>
				...
				<div className="flex w-full flex-1 items-center">
					...
					<div className="w-full cursor-pointer select-none border-edge border-l p-4">
						<div className="flex items-center justify-between [&_h2]:font-pixel-square [&_h2]:lowercase">
							<h2 className="text-left text-base sm:text-xl">{metadata.title}</h2>
			...
			<div className="screen-line-before flex items-center justify-between gap-2 px-2 py-2 sm:gap-4 sm:px-4">
				...
				<div className="flex items-center gap-2 sm:gap-4">
					...
					<Badge className="lowercase max-sm:hidden">créé: {dayjs(metadata.createdAt).format('DD MMM YYYY')}</Badge>
					<Badge className="lowercase">mis à jour: {dayjs(metadata.updatedAt).format('DD MMM YYYY')}</Badge>
				</div>
			</div>
