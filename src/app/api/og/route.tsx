...
export const GET = async (req: NextRequest) => {
	try {
		...
		const [fonts, badge] = await Promise.all([loadFonts(), Promise.resolve(getBadge(type))]);
		...
		return new ImageResponse(
			...
			>
				...
				>
					...
					<span style={{ color: '#71717B', marginLeft: 12 }}>{GLOBAL_DATA.WORK.title}</span>
				</div>
			</div>,
			{
				width: 1200,
				height: 630,
				fonts: [
					{ name: 'Geist Sans', data: fonts.regular, style: 'normal', weight: 400 },
					{ name: 'Geist Sans', data: fonts.medium, style: 'normal', weight: 600 },
					{ name: 'Geist Sans', data: fonts.bold, style: 'normal', weight: 800 },
				],
			}
		);
	} catch (error) {
