...
export const Cover = ({ loop = true }: CoverProps): React.JSX.Element => {
	...
	const [activeTab, setActiveTab] = useState<GreetingId>(GREETINGS_CONFIG[0].id);
	...
	const handleTabChange = (newTabId: string) => {
		const oldIndex = GREETINGS_CONFIG.findIndex((item) => item.id === activeTab);
									...
									{GREETINGS_FILE_TYPES.map(({ type, Icon, getLabel }) => (
										<ContextMenuItem
											...
											onClick={() => {
												...
												downloadFile(`/assets/${activeData.asset}/${type}/${filename}`, filename);
											}}
										>
